...
if strcmp(parameter.group_onoff, 'off')
    ...
    if strcmp(diff_onoff,'off')
        ...
        legend('oringnal train set','oringnal test set', 'estimated');
        title('Result: First iteration');xlabel('Time');ylabel('Output');
        ...
        legend('oringnal train set','oringnal test set', 'estimated');
        title('Result: End iteration');xlabel('Time');ylabel('Output');
        line([t(train_length+1) t(train_length+1)], [1.01*min(min(y)) 1.01*max(max(y))],...
            'LineWidth',2,'Color',[.8 .8 .8]);

...
function [c,k,res,mvmMfun,prec] = linsolve_lcg(A,R,B,P,b,kmax,c,tol,prec)
...
if isempty(prec) || prec
  fftdiag = ~isnumeric(A) && ~isnumeric(B);
  if fftdiag
    dX = diagFAtAFt(A,'cheap'); dB = diagFAtAFt(B,'cheap');
